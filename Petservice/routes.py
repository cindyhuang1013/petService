...
from Petservice.models import Pet, Owner, Petsitter, Review_sitter, Review_owner, Walk, Fostercare, Reservation
from Petservice.form import RegisterForm_owner, RegisterForm_pet, RegisterForm_petsitter, owner_LoginForm, sitter_LoginForm, OwnerReservationWalkForm, OwnerReservationFosterForm,editForm_owner,editForm_pet, editForm_sitter,commentForm_owner,commentForm_sitter
from Petservice import db
...
#Page for user to choose their identity (pet sitter or pet owner)
...
#The page for babysitter to manage their own accout (update their info / see the order they accept)
@app.route('/babysitter', methods=['GET'])
def babysitter():
    if current_user.is_authenticated:
        actual_user = current_user._get_current_object()
        if isinstance(actual_user, Petsitter):
            SitterID = actual_user.SitterID  
...
# The page for owner and sitter to update their current info
@app.route('/edit_profile', methods=['GET', 'POST'])
def edit_profile():
    actual_user = current_user._get_current_object()

    #if the user is pet owner
...
#The page for sitter to read the demand from pet owner 
...
#The page for sitter to review owner's past comments
...
#The page for sitter and pet owner to wirte their comments to each other
...
def comment():
    ...
    if request.method == 'POST':
        ...
        if isinstance(actual_user, Owner):
            ...
            form.reservationid.data= reservationid
...
#The page for owner to manage their own accout (see their current successful order / reserve new order / update info)
...
def owner_search_reservation_page(): 
    ...
    for petsitter, avg_rating in filtered_data:
        ...
        servicetype = petsitter.SitterServiceType  
        filter.append({
            'sitterid': sitterid,
...
#The page for owner to place walk order
...
#The page for owner to place foster order
...
#The page for owner to see if they order successfully
...
#The page for owner to read the sitter's past comments
